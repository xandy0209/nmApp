...
import { User, Activity, CheckCircle2, AlertCircle, Clock, ChevronRight, FileText } from 'lucide-react';
...
const GroupTaskList: React.FC<GroupTaskListProps> = ({ data, onItemClick }) => {
  if (data.length === 0) {
    return (
  ...
  return (
    <div className="space-y-3">
      {data.map((item: any) => (
        <div 
          key={item.id} 
          onClick={() => onItemClick && onItemClick('TASK', item.id)}
